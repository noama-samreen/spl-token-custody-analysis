...
def create_styles():
    ...
    return styles, title_style, cell_style, context_style, header_style, confidentiality_style, conflicts_style
...
def create_doc(token_data, output_dir):
    """Generate DOC for a single token"""
    # Handle missing or invalid name/symbol
    token_name = token_data.get('name', 'Unknown')
    if token_name in ['N/A', None, '']:
        token_name = 'Unknown'
    ...
    # Title
    title = doc.add_heading(f"Solana Token Security Assessment:\n{token_name} ({token_symbol})", 0)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    
    # Add confidentiality notice
    confidentiality = doc.add_paragraph("Confidential treatment requested under NY Banking Law § 36.10 and NY Pub. Off. Law § 87.2(d).")
    confidentiality.style = doc.styles['Caption']
    doc.add_paragraph()
    ...
    for i, (field, value) in enumerate(basic_data):
...
def generate_reports(token_data, output_dir):
    """Generate both PDF and DOC reports"""
